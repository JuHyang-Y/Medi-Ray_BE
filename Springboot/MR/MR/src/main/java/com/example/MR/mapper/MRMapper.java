...
public interface MRMapper {
		...
		@Insert("INSERT INTO DOCTOR_TB(DT_CODE, DT_ID, DT_PW, DT_NAME, DIVISION, DT_TELNO) "
				+ "VALUES(#{dtCode}, #{dtId}, #{dtPw}, #{dtName}, #{division}, #{dtTelno})")
		...
		@Select("SELECT * FROM DOCTOR_TB WHERE DT_ID = #{dtId}")
		public ArrayList<DoctorTb> dtSelect(String dtId);
		// 마이페이지입력값 수정(비밀번호 변경가능)
		@Update("UPDATE DOCTOR_TB SET DT_PW = #{dtPw} "
				+ "WHERE DT_ID = #{dtId}")
		public void updateDoctorPw(String dtId, String dtPw);
		// 마이페이지입력값 수정(의사소속 변경가능)
		@Update("UPDATE DOCTOR_TB SET DIVISION = #{division} "
				+ "WHERE DT_ID = #{dtId}")
		public void updateDivision(String dtId, String division);
		
		// 업로드 페이지
		// 같은 환자 번호를 가지고 있는 의사의 이름 불러오기
		@Select("SELECT d.DT_NAME AS dtName FROM DOCTOR_TB d "
		        + "JOIN PATIENT_TB p ON p.DT_CODE = d.DT_CODE  "
		        + "WHERE p.PT_CODE = #{ptCode} ")
		public String DtName(String ptCode);
		// 환자에 대한 의사코드 매칭
		@Select("SELECT DT_CODE FROM PATIENT_TB WHERE PT_CODE = #{ptCode}")
		public String DtCode(String ptCode);
...
//		@Insert("INSERT INTO PATIENT_TB(PT_CODE, PT_NAME, PT_BIRTHDATE, PT_GEN, DT_CODE")
		...
		@Select("SELECT p.*, d.DT_NAME AS dtName, i.XRAY_CODE "
				+ "FROM PATIENT_TB p "
				+ "JOIN IMG_TB i ON p.PT_CODE = i.PT_CODE "
				+ "JOIN DOCTOR_TB d ON p.DT_CODE = d.DT_CODE "
				+ "WHERE (p.PT_NAME LIKE CONCAT('%', #{ptName}, '%') OR p.PT_CODE = #{ptNameOrCode}) "
				+ "AND i.XRAY_DATE = ( "
