...
public interface MRMapper {
	...
	@Insert("INSERT INTO DOCTOR_TB(DT_CODE, DT_ID, DT_PW, DT_NAME, DIVISION, DT_TELNO) "
			+ "VALUES(#{dtCode}, #{DT_PW}, #{DT_PW}, #{dtName}, #{division}, #{dtTelno})")
	public void registDoctor(DoctorTb dt);

	// 로그인, 의사테이블에서 중복된 id가 있는지 확인하기
	@Select("SELECT DT_CODE, DT_ID, DT_PW, DT_NAME, DIVISION, DT_TELNO FROM DOCTOR_TB WHERE BINARY DT_ID = #{dtId}")
