...
(function () {
  ...
  this.Waves = function (holder, options) {
    ...
    Waves.canvas.style.position = 'fixed';
    Waves.canvas.style.top = 0;
    Waves.canvas.style.left = 0;
    Waves.canvas.style.zIndex = '-1'; // 기존 콘텐츠보다 뒤에 위치
    document.body.appendChild(Waves.canvas);
  ...
  Waves.prototype.render = function () {
    ...
    if (Waves.options.debug) {
      ctx.beginPath();
      ctx.strokeStyle = '#f00';
      ctx.arc(Waves.centerX, Waves.centerY, Waves.radius, pi2);
      ctx.stroke();
    }
...
var waves = new Waves('#holder', {
  fps: false, // FPS 표시 원하지 않으면 false로 설정
  waves: 3,
  width: 200,
  hue: [200, 220], // 원하는 색상 범위로 조정
  amplitude: 0.3, // 파도의 높이 조절
  speed: [0.002, 0.004], // 파도의 속도 조절
  background: false, // 배경색을 원하지 않으면 false
});
